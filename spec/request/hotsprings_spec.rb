RSpec.describe 'Hotspring', type: :request do
  ...
  describe 'Post /api/hotsprings' do
    it 'is valid with correct params' do
      hot = build(:hotspring)
      post '/api/hotsprings',
           { params: { name: hot.name, 'latitude': hot.latitude, 'longtitude': hot.longtitude,
    ...
    it 'is valid with empty name' do
      hot = build(:hotspring)
      post '/api/hotsprings',
           { params: { 'latitude': hot.latitude, 'longtitude': hot.longtitude, 'description': hot.description } }
      expect(response).to have_http_status(200)
  ...
  describe 'Patch /api/hotsprings' do
    it 'is valid with correct params' do
      hot = hotspring
      patch '/api/hotsprings/' + String(hot.id),
            { params: { name: 'Update', 'latitude': hot.latitude, 'longtitude': hot.longtitude,
                        'description': hot.description } }
      expect(response).to have_http_status(200)
