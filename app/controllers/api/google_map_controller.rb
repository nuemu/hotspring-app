class Api::GoogleMapController < Api::BaseController
  ...
  def nearby_search(lat, lng, type)
    url = URI(`
              ...
              ?location=#{lat}%2C#{lng}&radius=1500&language=ja
              &type=#{type}&key=#{ENV['GOOGLE_PLACES_API_KEY']}
            `)
    ...
    JSON.parse(response.read_body)['results'].map do |result|
      photo_url = nil
      if result['photos']
        photo_url = URI(`
          https://maps.googleapis.com/maps/api/place/photo
          ?maxwidth=400&photo_reference=#{result['photos'][0]['photo_reference']}&language=ja
      ...
      NearbyPlaces.new(
        ...
        result['name'],
        photo_url ? photo(photo_url) : 'none'
      )
    end
  end
